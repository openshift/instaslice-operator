...
// is already present (e.g., from a source webhook like Job or PyTorchJob), it's not re-injected.
func TestMutatePodGPUMemoryAlreadyInjected(t *testing.T) {
	hook := &InstasliceWebhook{}

	// Simulate a Pod that came from a Kueue Workload where the source webhook
			...
			Containers: []corev1.Container{
				{
					...
					Resources: corev1.ResourceRequirements{
						Limits: corev1.ResourceList{
							...
							corev1.ResourceName("nvidia.com/mig-1g.5gb"): resource.MustParse("1"),
							// GPU memory already injected by source webhook
	...
	// GPU memory should remain at 10 (from source webhook), not be overwritten to 5
	...
	// The value should be preserved from the original (10GB from source webhook)
	// not recalculated to 5GB from the single 1g.5gb profile
	if gpuMem.Value() != 10 {
		t.Fatalf("expected GPU memory to remain at 10GB (from source webhook), got %d", gpuMem.Value())
	}
