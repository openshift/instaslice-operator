...
func TestMutatePodNvidiaResource(t *testing.T) {
	...
	q, ok := limits[corev1.ResourceName("instaslice.com/mig-1g.5gb")]
	if !ok || q.Value() != 1 {
...
func TestMutatePodInstaResource(t *testing.T) {
			...
			Containers: []corev1.Container{
				{
					...
					Resources: corev1.ResourceRequirements{
						Limits: corev1.ResourceList{
							corev1.ResourceName("instaslice.com/mig-1g.5gb"): resource.MustParse("1"),
						},
					},
				},
			},
	...
	if _, ok := mutated.Spec.Containers[0].Resources.Limits[corev1.ResourceName("instaslice.com/mig-1g.5gb")]; !ok {
