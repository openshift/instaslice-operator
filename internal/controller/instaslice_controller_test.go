...
// Test `gpuAllocatedSlices`
func TestGpuAllocatedSlices(t *testing.T) {
	Describe("gpuAllocatedSlices", func() {
		It("should return correct allocation indexes", func() {
			instaslice := &inferencev1alpha1.Instaslice{
				Status: inferencev1alpha1.InstasliceStatus{
					PodAllocationResults: map[types.UID]inferencev1alpha1.AllocationResult{
						"uid1": {
			...
			expected := []int32{0, 1, 1, 0, 0, 0, 0, 0}
			Expect(gpuAllocatedSlices(instaslice, "gpu-1")).To(Equal(expected))
		})
	})
}

// Test `getFreeAndUsedSlicesCount`
func TestGetFreeAndUsedSlicesCount(t *testing.T) {
	Describe("getFreeAndUsedSlicesCount", func() {
		It("should correctly count free and used slices", func() {
			instaslice := &inferencev1alpha1.Instaslice{}
			expectedFree := int32(7) // Skipping index 7
			expectedUsed := int32(0)
			freeSlices, usedSlices := getFreeAndUsedSlicesCount(instaslice, "gpu-1")
			Expect(freeSlices).To(Equal(expectedFree))
			Expect(usedSlices).To(Equal(expectedUsed))
		})
	})
}
...
func TestGetStartIndexFromPreparedState(t *testing.T) {
	Describe("getStartIndexFromPreparedState", func() {
		It("should return the correct start index for a profile", func() {
			...
			gpuAllocatedIndex := []int32{1, 0, 0, 0, 0, 0, 0, 0}
			Expect((&InstasliceReconciler{}).getStartIndexFromPreparedState(instaslice, "2g.10gb", gpuAllocatedIndex)).To(Equal(int32(1)))
		})
	})
}

// Test `actualProfileSliceSize`
func TestActualProfileSliceSize(t *testing.T) {
	Describe("actualProfileSliceSize", func() {
		It("should return the correct actual slice size allocated", func() {
			...
			Expect((&InstasliceReconciler{}).actualProfileSliceSize(instaslice, "4g.20gb", "gpu-1")).To(Equal(int32(4)))
		})
	})
}

// Test `getTotalFitForProfileOnGPU`
func TestGetTotalFitForProfileOnGPU(t *testing.T) {
	var _ = Describe("GetTotalFitForProfileOnGPU", func() {
		...
		for _, tt := range tests {
			It(tt.name, func() {
				r := &InstasliceReconciler{}
				fit := r.getTotalFitForProfileOnGPU(tt.instasliceObj, tt.profileName, tt.gpuuid)
...
var _ = Describe("Metrics Incrementation", func() {
	...
	It("should not increment metrics more than once", func() {
		...
		err := r.IncrementTotalProcessedGpuSliceMetrics(*instaslice, "node-1", "gpu-1", "1g.5gb")
