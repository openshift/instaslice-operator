...
func TestGpuAllocatedSlices(t *testing.T) {
	Describe("gpuAllocatedSlices", func() {
		It("should return correct allocation indexes", func() {
			...
			Expect((&InstasliceReconciler{}).gpuAllocatedSlices(instaslice, "gpu-1")).To(Equal(expected))
		})
	})
}
...
var _ = Describe("calculateProfileFitOnGPU", func() {
	...
	It("should return the correct actual slice size allocated", func() {
		Expect(r.calculateProfileFitOnGPU(instaslice, "4g.20gb", "gpu-1", false)).To(Equal(int32(4)))
	})
})
...
func TestGetTotalFitForProfileOnGPU(t *testing.T) {
	var _ = Describe("GetTotalFitForProfileOnGPU", func() {
		...
		for _, tt := range tests {
			It(tt.name, func() {
				r := &InstasliceReconciler{}
				fit, err := r.calculateProfileFitOnGPU(tt.instasliceObj, tt.profileName, tt.gpuuid, true)
