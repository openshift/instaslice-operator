...
func (r *InstasliceReconciler) updateMetrics(ctx context.Context, instasliceList inferencev1alpha1.InstasliceList) error {
	...
	for _, instaslice := range instasliceList.Items {
		...
		for _, gpu := range instaslice.Status.NodeResources.NodeGPUs {
			...
			if !allocated {
				totalSlots, err := r.updateGpuSliceMetricsForUnallocatedGPU(ctx, instaslice, nodeName, gpuID)
				if err != nil {
					log.Error(err, "Failed to update GPU slice metrics for unallocated GPU", "nodeName", nodeName, "gpuID", gpuID)
					return err
				}
...
func (r *InstasliceReconciler) updateGpuSliceMetricsForUnallocatedGPU(ctx context.Context, instaslice inferencev1alpha1.Instaslice, nodeName, gpuID string) (int32, error) {
	totalSlots, err := r.getTotalGpuSlotsForGPU(instaslice, gpuID)
	if err != nil {
...
func calculateUsedSlotsForGPU(instaslice inferencev1alpha1.Instaslice, nodeName, gpuID string) int32 {
	usedSlots := int32(0)
	for podUid, allocation := range instaslice.Status.PodAllocationResults {
		...
		if string(allocation.Nodename) == nodeName && allocation.GPUUUID == gpuID {
			if string(allocation.Nodename) == nodeName && allocation.GPUUUID == gpuID {
				if allocation.MigPlacement.Size == 8 { // handle for 7g.40gb profile
					usedSlots += maxSlices7g40gb
				} else if allocRequest.Profile == profile3g20gb && allocation.MigPlacement.Start == EndStartPos3g20gb { // fills gpu and it doesnt need an extra slice
					usedSlots += EndPosSlices3g20gb
				} else if allocRequest.Profile == profile1g10gb && allocation.MigPlacement.Start == EndStartPos1g10gb { // fills gpu and it doesnt need an extra slice
					usedSlots += EndPosSlices1g10gb
				} else {
