...
- create a base CSV file that contains metadata, the base CSV file name can be
arbitrary, we can follow the convention `operator-base-clusterserviceverison.yaml`
...
$ tree deploy/
  ...
  ├── base-csv
  │   └── operator.base.clusterserviceversion.yaml
  ├── deployment.yaml
  ├── role.yaml
  ├── role_binding.yaml
...
The operator-sdk doc does not go over base csv criteria, the details can be found by
